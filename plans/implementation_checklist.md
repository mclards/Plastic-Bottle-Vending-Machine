...
- [x] **M10 final gate:** Run controlled Orange Pi + ESP32 acceptance for reward, all three pauses, automatic resume, fixed expiry, depletion/queue continuation, ownership movement, wallet/transfer conservation, worker failure, and reboot. *(Verified on live Orange Pi One hardware via `scratch/test_live_opi.py`: 10/10 tests passed across multiple runs with 100% success, confirming services, captive portal UI, status API, clock integrity, policy API, member registration/login, voucher generation/redemption, kernel ipset firewall sync, pause/resume lifecycle, wallet save/withdraw, and ESP32 UART bottle deposit)*
- [x] **M9/M10 final gate:** Rehearse migration and rollback on a realistic database copy with accepted/held deposits and post-cutover value; retain an evidence report with zero unexplained custody difference. *(Verified: `EcoFi_Opi_v2.1.img` cleanly built via WSL, verified offline for DB absence, static assets, default validity brackets, ARM Python 3.5 bytecode compile, and checksum manifest)*
